...
| h5000m IPv6 shared-prefix route reconciler | PROJECT_LOCAL | Run 22 prebuild implementation 1 | This repository; exact implementation commit recorded by Git history | rescue/full through existing `higoros`; correct confirmed RFC7278 same-prefix return-route collision | Exact `wwan0_1` iface lifecycle hook plus `/usr/libexec` helper; owns only `br-lan` metric `1` protocol `242`; runtime-only lock/state; no upstream/network/firewall/modem patch | 16 deterministic fixtures, shell syntax, double apply, exact executable install, ownership and prohibited-mutation gates PASS; firmware build not yet run | Device validation UNVERIFIED pending exact Run 22 RAM route/function/lifecycle contract | Change only after RCA/design review; require clean build, exact-run RAM regression, renewal/reconnect lifecycle, rollback and recovery evidence |
