...
| h5000m IPv6 shared-prefix route reconciler | PROJECT_LOCAL | Run 22 implementation 1 | This repository at build-input project SHA `ca75cc3d6a9b7ce1907656d585fa1c5b283c030b` | rescue/full through existing `higoros`; correct confirmed RFC7278 same-prefix return-route collision | Exact `wwan0_1` iface lifecycle hook plus `/usr/libexec` helper; owns only `br-lan` metric `1` protocol `242`; runtime-only lock/state; no upstream/network/firewall/modem patch | Run 22: 16 deterministic fixtures, shell syntax, double apply, exact executable install, ownership/prohibited-mutation gates, compile and firmware byte inspection PASS; helper/hook exact SHA256 recorded in Run 22 ledger | Device validation UNVERIFIED pending exact Run 22 RAM route/function/lifecycle contract | Change only after RCA/design review; require exact-run RAM regression, renewal/reconnect lifecycle, rollback and recovery evidence |
