...
|---|---|---|---|---|---|---|---|---|
| higoros | VENDOR_HIGO | 1.26.04.29.09-r1 | canonical local payload; hashes in candidate lock | rescue/full; vendor UI/API | PROJECT_LOCAL packaging, defaults, init and Higo:80/LuCI:8080 coexistence | Run 20 PASS; payload and installed-tree hashes PASS | Run 20 dashboard/CPE, safe `ATI`, logs, runtime tasks and terminal PASS; notifications operations fail `not found`; neighbour/profile mapping gaps; Full-only widgets lack data | Hash-pin; manual candidate change with compatibility/build/device evidence |
