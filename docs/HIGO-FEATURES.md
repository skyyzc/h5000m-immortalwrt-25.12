...
| 5G status | yes | Run 21 resolver/computed/UI title shows `4G + 5G` | rescue | QModem/Higo | FUNCTION_TESTED_RUN21 | Exact Run 21 paused state proved `mode=auto`, selected `[4G,5G]`, resolver title `4G + 5G`, computed badge `4G / 5G`; resumed UI displayed the same title. Earlier unknown-title evidence is retained as a non-reproduced timing/state discrepancy, not a confirmed cache/API/resolver defect | no additional CPE repair; retain regression coverage in the next authorized exact-run validation |
