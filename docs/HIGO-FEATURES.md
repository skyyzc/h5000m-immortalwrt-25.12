...
| 5G status | yes | Run 20 network type and signal read successfully | rescue | QModem/Higo | FUNCTION_TESTED_RUN20 / RUN21_REPAIR_FAIL | Run 21 real-device evidence retained QModem `3G=0,4G=1,5G=1` and showed a `4G / 5G` badge, but the current-configuration title still rendered `未识别配置`; the built display-only normalization therefore did not close the live frontend path | inspect the exact Run 21 loaded asset/runtime presentation path before proposing a minimal follow-up repair |
