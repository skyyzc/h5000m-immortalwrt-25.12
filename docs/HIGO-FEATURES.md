...
| 5G status | yes | Run 20 network type and signal read successfully | rescue | QModem/Higo | FUNCTION_TESTED_RUN20 / RUN21_REPAIR_FAIL | CPE-RUN21-LIVE-BODY-01 proves the browser response contains the Run 21 patched resolver, rejecting stale canonical cache selection; its curated excerpt cannot establish the full loaded SHA256. The patched code still produced the unknown visible title, so live Vue resolver state remains unproved and `CPE_RCA=UNKNOWN` | capture `F.value.mode`, `F.value.selectedNetworks`, normalized key, `Ge.value`, `st.value`, and `Tt.value` in one bounded read-only debugger session; do not select another repair before this closes |
