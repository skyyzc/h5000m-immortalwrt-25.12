...
| 5G status | yes | Run 20 network type and signal read successfully | rescue | QModem/Higo | FUNCTION_TESTED / PATCH_PREPARED | DEVICE-01F-E2 proves QModem `3G=0,4G=1,5G=1` becomes authenticated API `mode=auto`, `selectedNetworks=[4G,5G]`; DEVICE-01F-II adds a deterministic display-only `4G + 5G` normalization patch without modem/backend changes | Run 21 build then RAM regression; current patched runtime remains UNVERIFIED |
