...
| 5G status | yes | Run 20 network type and signal read successfully | rescue | QModem/Higo | FUNCTION_TESTED_RUN20 / RUN21_REPAIR_FAIL | CPE-EVIDENCE-01 captured unchanged authenticated API/QModem semantics, the actual browser request URL, and the Run 21 patched bytes currently served there; a new isolated tab still showed the unknown title. The browser interface did not expose the already loaded body/hash, so cached canonical selection versus patched-resolver runtime input remains unresolved and `CPE_RCA=UNKNOWN` | obtain the exact browser response-body hash or equivalent evaluated-code/input proof; do not select another repair before this closes |
...
| IPv6 | yes | Run 21 device-side IPv6 works; real Wi-Fi client IPv6 fails | rescue | netifd/odhcpd/QMAP | PARTIAL / CLIENT_FAIL / ROOT_CAUSE_CONFIRMED | bounded Run 21 capture saw each client echo request on `br-lan` and `wwan0_1`, matching replies on `wwan0_1`, and no reply on `br-lan`; the same delegated `/64` exists on both interfaces and route lookup sends the LAN-client destination back to `wwan0_1`, confirming a local return-route prefix collision rather than absent carrier replies | separate owner-reviewed repair design; do not apply routing, firewall, NDP, RA or QMAP changes during validation |
