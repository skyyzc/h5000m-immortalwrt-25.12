...
| 5G status | yes | Run 20 network type and signal read successfully | rescue | QModem/Higo | FUNCTION_TESTED_RUN20 / RUN21_REPAIR_FAIL | DEVICE-01G proves one canonical title resolver and one chunk reference; Run 21 patched that chunk but retained its filename, while live behavior matched the canonical resolver. Cache/already-loaded module selection is the leading hypothesis, but the browser-loaded bytes were not captured, so `CPE_RCA=UNKNOWN` | one fresh private-context Run 21 session: sanitized API, loaded chunk URL/hash/cache metadata, and visible title; select repair only after this closes the live path |
